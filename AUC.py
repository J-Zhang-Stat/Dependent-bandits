...
if __name__=="__main__":
    ...
    auc_ucb = np.trapz(avg_regret_ucb, dx=1)
    auc_ucbc = np.trapz(avg_regret_ucbc, dx=1)
    auc_bic = np.trapz(avg_regret_bic, dx=1)

    print("AUC(Area Under Curve) for Average Cumulative Regret")
    print(f"UCB    : {auc_ucb:.2f}")
    print(f"UCB-C  : {auc_ucbc:.2f}")
    print(f"BIC-UCB: {auc_bic:.2f}")