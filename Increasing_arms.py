...
# ---------------------------------------------
# 1. Environment and Distribution Class Definitions
# ---------------------------------------------
class NormalCluster:
...
# ---------------------------------------------
# 2. Main Function: Investigating BIC-UCB's Regret When num_arms Increases in One Cluster
# ---------------------------------------------
if __name__ == "__main__":
    # Keep settings for other clusters fixed
    # Assume we experiment on the first cluster (cluster_id=0) and observe the impact of increasing num_arms
    # You can change it to another cluster_id if needed

    base_num_arms = [2, 2, 3, 2, 3]   # Base settings for other clusters
    theta_array = [0.1, 0.5, 0.2, 0.3, 0.1]
    sigma_array = [1, 1, 1, 1, 1]

    # Prepare a set of num_arms to test (e.g., 2, 4, 6, 8, 10)
    arms_candidate = [2, 4, 6, 8, 10]

    T = 3000  # Number of steps per experiment
    ...
    # Store average regret curves for different arms configurations
    all_regrets = []

    plt.figure(figsize=(10, 6))

    for arms_val in arms_candidate:
        # Modify the number of arms for the 0th cluster
        new_num_arms = base_num_arms.copy()
        new_num_arms[1] = arms_val

        # Run multiple experiments and take the average
        cum_reg_list = []
        ...
        all_regrets.append(avg_reg)

        # Plot the average regret curve for this arms_val
        plt.plot(range(T), avg_reg, label=f'arms={arms_val}')

    plt.xlabel('Time Steps')
    plt.ylabel('Cumulative Regret')
    plt.title('BIC-UCB: Impact of Increasing num_arms in One Cluster')
    plt.legend()
    plt.tight_layout()
    plt.show()