...
def compute_regret_difference(avg_regret_ucbc, avg_regret_bic, stable_start=5000):

    # Absolute difference in regrets
    regret_diff_full = np.abs(avg_regret_bic - avg_regret_ucbc)
    # Percentage difference based on absolute difference
    percentage_difference_full = (regret_diff_full / avg_regret_ucbc) * 100.0
    
    # Slice from stable_start
    regret_diff_stable = regret_diff_full[stable_start:]
    percentage_diff_stable = percentage_difference_full[stable_start:]
    return regret_diff_stable, percentage_diff_stable

def plot_regret_comparison(T, avg_regret_ucbc, avg_regret_bic, stable_start=5000):
    """
    Plots the absolute percentage difference from T >= stable_start onward.
    """
    regret_diff, percentage_difference = compute_regret_difference(avg_regret_ucbc, avg_regret_bic, stable_start)
    time_range = range(stable_start, T)

    plt.figure(figsize=(10, 5))
    plt.plot(time_range, percentage_difference, label='Absolute Percentage Difference (BIC-UCB vs UCB-C)')
    plt.xlabel('Time Steps (>=5000)')
    plt.ylabel('Percentage Difference (%)')
    plt.title('Absolute Percentage Difference in Cumulative Regret, T >= 5000')
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ...
    percentage_diff_cumulative = plot_regret_comparison(T, avg_regret_ucbc, avg_regret_bic, stable_start=5000)
